...
import { fetchWeatherReading } from './weatherReaderSlice'
...
function WeatherReader() {
    ...
    const {
        ...
        humidity,
        initialized
    } = UseAppSelector((state) => state.weatherReader );

    useEffect(() => { 

        dispatch(fetchWeatherReading());

        // Fetch new data every 30 seconds
        const intervalId = setInterval(() => dispatch(fetchWeatherReading()), 1000 * 30);
    ...
    return (
        <div className="shadow rounded p-5 text-center bg-gray-800">
            ...
            <div className="text-white py-3" id='WeatherDisplay'>
                ...
                <div className="mb-4">
                    <p id="Temp">{(initialized ? `${temp}°F` : "Loading Weather Data...")}</p>
                </div>
                <p className="title">Humidity</p>
                <div>
                    <p id="Humidity">{(initialized ? `${humidity}%` : "Loading Weather Data...")}</p>
                </div>            
            </div>
        </div>
    );
