...
import { makeRfCall } from './rfRemoteSlice'

function renderButton(title:string, switchNumber:number) {
    ...
    return (
        <div className="py-5 px-3">
            ...
            <div className="grid grid-cols-2 gap-4 mx-3" id={`${title}-switchBlock`}>
                <button className="bg-green-300 px-4 py-2 rounded text-green-900 uppercase font-bold" onClick={() => { dispatch(makeRfCall({ "switchNumber": switchNumber, "on": true }))}}>
                    On
                </button>
                <button className="bg-red-400 px-4 py-2 rounded text-red-900 uppercase font-bold" onClick={() => { dispatch(makeRfCall({ "switchNumber": switchNumber, "on": false }))}}>
                    Off
                </button>
            </div>
        </div>
...
function RfRemote() {
    return (
        <div className="shadow rounded p-5 text-center bg-gray-800 my-5">
            ...
            <div id='rfRemote'>
                {renderButton("Living Room Fan", 1)}
                {renderButton("Living Room Lights", 2)}
                {renderButton("Bed Room Lights", 3)}
            </div>
        </div>
    );
}
